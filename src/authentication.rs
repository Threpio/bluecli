use oauth2::{basic::BasicClient, EmptyExtraDeviceAuthorizationFields, AuthUrl, ClientId, DeviceAuthorizationResponse, DeviceAuthorizationUrl, Scope, TokenResponse, TokenUrl};
use reqwest::{Client};
use tokio::time::sleep;

pub(crate) async fn auth() -> anyhow::Result<()> {
    // Use Microsoft's public Azure CLI client ID
    let azure_cli_client_id = "04b07795-8ddb-461a-bbee-02f9e1bf7b46";

    let client = BasicClient::new(
        ClientId::new(azure_cli_client_id.to_string()),
    )
        ...
        .set_auth_uri(AuthUrl::new("https://login.microsoftonline.com/common/oauth2/v2.0/authorize".parse().unwrap())?)
        .set_token_uri(TokenUrl::new("https://login.microsoftonline.com/common/oauth2/v2.0/token".parse().unwrap())?);

    // Start the device code flow
    let details: DeviceAuthorizationResponse<EmptyExtraDeviceAuthorizationFields> = client
        .exchange_device_code()
        .add_scope(Scope::new("https://management.azure.com/.default".to_string()))
